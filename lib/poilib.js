...
class PoiCont {
    ...
    getPois(targets, allPoi = false) {
        ...
        const wantSet = allowAll ? null : new Set(isArray ? targets : [String(targets)]);

        geojson.forEach((feat, idx) => {
            const tgts = poiCont.pdata.targets?.[idx] || [];
            for (const t of tgts) {
                const inTarget = allowAll || wantSet.has(t);    // allPoi=true なら常に true。activity は従来互換で常に true。
    ...
    setPoi(flist, actonly) {
        ...
        const saveMarker = function (params) {              // params内のgeojsonをgeojson変数に保存
            ...
            if (params.actlists.length > 0 || isOpenNow) {
                ...
                css_name = "normal"
            } else if (poiStatus[PoiStatusIndex.VISITED]) {
                css_name = Conf.etc.reverseIcon ? "normal" : "visited"
            }
